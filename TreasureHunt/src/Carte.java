	...
	public void mouvePersonage(Personnage perso, int pos_x, int pos_y) {
		...
		if (!(joueurs[0].posX_Bateau == pos_x && joueurs[0].posY_Bateau == pos_y
			...
			if(perso.tresor){
				...
				JOptionPane.showMessageDialog(null, "Le joueur "+(perso.numEquipe+1)+" a gagnï¿½ !");
			}
		...
		if(perso.cle && test.contenu instanceof Coffre){
			...
			if (((Rocher) test.contenu).cleIci) {
				if (((Rocher) test.contenu).cle) {
					...
					perso.cle = true;
					JOptionPane.showMessageDialog(null, "J'ai trouvï¿½ la clï¿½ !");
				} else {
				...
				if (((Rocher) test.contenu).coffre) {
					if (perso.cle) {
						...
						JOptionPane.showMessageDialog(null,
								"J'ai rï¿½cupï¿½rï¿½ le trï¿½sor, cassons nous de cette ï¿½le maudite !");
					} else {
						JOptionPane.showMessageDialog(null,
								"J'ai trouvï¿½ le coffre, je pense qu'il est bloquï¿½ par un mï¿½canisme ancien de type serrure ï¿½ clï¿½ !");
					}
				} else {
					JOptionPane.showMessageDialog(null, "J'ai trouvï¿½ le coffre ouvert... Mais il est vide !");
				}
			} else {
				Random rdm = new Random();
				String[] punchline = { "Je ne vois que des insectes lï¿½ dessous !",
						"Il n'y a rien dedans comme dans la tï¿½te du guerrier ! ",
						"C'est le vide lï¿½ dessous, comme mon compte en banque d'ailleurs !",
						"C'est vide, comme l'inspiration de mon dï¿½veloppeur !" };
			...
			if (((Personnage)test.contenu).numEquipe != perso.numEquipe) {
				if (((Personnage) test.contenu).cle) {
					...
					JOptionPane.showMessageDialog(null, "J'ai volï¿½ la clï¿½, trouvons le trï¿½sor !");
				} else if (((Personnage) test.contenu).tresor) {
					perso.tresor = true;
					((Personnage) test.contenu).tresor = false;
					JOptionPane.showMessageDialog(null, "J'ai volï¿½ le trï¿½sor, nous pouvons s'en aller !");
				} else {
				...
				if (perso.cle && ((Personnage)test.contenu).typePerso == Personnage.EXPLORATEUR) {
					...
					JOptionPane.showMessageDialog(null, "Tiens mon pote, prends la clï¿½ !");
					perso.energie -= 20;
				}else{
			...
			if (((Rocher) test.contenu).cle) {
				...
				JOptionPane.showMessageDialog(null, "J'ai cassé le rocher, je peux y voir le coffre ! !");
