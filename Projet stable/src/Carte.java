...
public class Carte {
	...
	public void genCarte(int taille) {
		...
		do {
			...
			parcelles[pos_x][1].addContenu(bateau[0], false);
			
			pos_x = random.nextInt(taille - 4) + 2;
			bateau[1] = new Bateau(Bateau.BATEAUDos, pos_x, taille - 2);
			parcelles[pos_x][taille - 2].addContenu(bateau[1], false);
	...
	private boolean genDone() {
		...
		tableau[0][0] = 1; // ceci représente un joueur virtuel qui va
							// parcourir
		...
		while (estBloque) { // tant que notre "personne" a bougé on essaie
			...
			for (int i = 0; i < tableau.length; i++) {
				for (int j = 0; j < tableau.length; j++) {
					...
					if (tableau[i][j] == 1) {
						if (this.parcelles[i + 2][j + 1].traversable && tableau[i + 1][j] == 0) { // si
																							// la
																							// case
																							// du
																							// dessus
							...
							tableau[i + 1][j] = 1; // on note la case par
													...
													// là
							estBloque = false; // nous ne sommes pas bloqués
												// car
												// une case s'est trouvée
												// accessible par le joueur.
						}
						if (this.parcelles[i][j + 1].traversable && tableau[i - 1][j] == 0) { // si
																						// la
																						// case
																						// du
																						// dessous
						...
						if (this.parcelles[i + 1][j + 2].traversable && tableau[i][j + 1] == 0) { // si
																							// la
																							// case
																							// de
																							// droite
						...
						if (this.parcelles[i + 1][j].traversable && tableau[i][j - 1] == 0) { // si
																						// la
																						// case
																						// de
																						// gauche
